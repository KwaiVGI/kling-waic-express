...
async function fetchPinedImage() {
  try {
...
// 获取展示图片（带重试机制）
async function fetchCastingImages() {
...
onUnmounted(() => {
  ...
  if (retryTimer.value)
    clearInterval(retryTimer.value)
  if (pinedImageCheckTimer.value)
    clearInterval(pinedImageCheckTimer.value)
